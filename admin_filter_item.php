                ...
                <div style="overflow-x:auto;" id="filterresult">
                    <table id="table_id1" class="display">
                        ...
                        <tbody>
                            ...
                            <?php
                            $sql = "SELECT item.itemid, item_name, quantity, sum(qty_transferred) as totaltransfer, sum(available_qty) as available_qty,  month, year FROM item, category, item_transfer, item_last_checked , item_state WHERE category.categoryID=item.categoryID AND item.itemid=item_transfer.itemID AND item_transfer.recordno=item_state.recordno AND item_state.checkedID = item_last_checked.checkedID AND category.categoryID = 1 AND month='january' AND year =2023 GROUP BY item.itemid";
    ...
    <script>
        $(document).ready(function() {
            $('#filterbutton').click(function() {
                ...
                $.ajax({
                    url: 'filter/admin_filter_item.php', // URL of the server-side script
